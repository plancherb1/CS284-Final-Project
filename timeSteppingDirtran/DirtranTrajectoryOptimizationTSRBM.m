classdef DirtranTrajectoryOptimizationTSRBM < DirectTrajectoryOptimization
  ...
  methods (Access=protected)
    function [f,df] = forward_constraint_fun(obj,h,x0,x1,u)
      ...
      [f,df] = dirtranConditionTSRBM(TSRBM,1,x0,x1,u);
      %}
      ...
      options.steps = steps;
      [f,df] = dirtranConditionTSRBM(TSRBM,4,x0,x1,u,options);
      %{%}
    ...
    function [f,df] = midpoint_constraint_fun(obj,h,x0,x1,u0,u1)
      ...
      catch % for TSRBM need to do the update not dynamics (assumes dt = h)
        options.u1 = u1;
        [f,df] = dirtranConditionTSRBM(TSRBM,3,x0,x1,u0,options);
      end
    end
